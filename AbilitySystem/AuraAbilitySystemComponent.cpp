...
void UAuraAbilitySystemComponent::AddCharacterAbilities(const TArray<TSubclassOf<UGameplayAbility>>& StartupAbilities)
{
	...
	{
		AddCharacterAbility(AbilityClass, 1);
	}
...
void UAuraAbilitySystemComponent::AddCharacterAbility(TSubclassOf<UGameplayAbility> AbilityClass, int32 AbilityLevel)
{
...
void UAuraAbilitySystemComponent::AbilityInputTagHeld(const FGameplayTag& InputTag)
{
	...
	for (int32 i = 0; i < AbilitySpecs.Num(); i++)
	{
		FGameplayAbilitySpec& AbilitySpec = AbilitySpecs[i];
		if (AbilitySpec.DynamicAbilityTags.HasTagExact(InputTag))
		{
			AbilitySpecInputPressed(AbilitySpec);
			if (!AbilitySpec.IsActive())
...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			// Remove this InputTag (Slot) from any ability tha has it.
		...
		ClientEquipAbility(AbilityTag, AuraGameplayTags.Abilities_Status_Equipped, Slot, PrevSlot);
	}
}
